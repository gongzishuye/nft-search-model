    ...
    def forward(self, x):
        ...
        for i in range(self.num_levels):
            ...
            if self.pool_type == 'max_pool':
                tensor = F.max_pool2d(x, kernel_size=kernel_size, stride=stride, padding=pooling).view(bs, -1)
            else:
                tensor = F.avg_pool2d(x, kernel_size=kernel_size, stride=stride, padding=pooling).view(bs, -1)
...
class FaceNetModel(nn.Module):
    def __init__(self, pretrained=False):
        ...
        self.model.fc = nn.Sequential(
            ...
            # nn.BatchNorm1d(1024),
            # nn.ReLU(),
            nn.Linear(204800, embedding_size))
