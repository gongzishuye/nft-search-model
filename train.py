...
def train_valid(model, optimizer, triploss, scheduler, epoch, dataloaders, data_size):
    for phase in ['train', 'valid']:
        ...
        for batch_idx, batch_sample in tqdm.tqdm(enumerate(dataloaders[phase])):
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                all = (neg_dist - (3 * pos_dist) < args.margin).cpu().numpy().flatten()
                if phase == 'train':
                ...
                print(batch_idx, triplet_loss.item())
